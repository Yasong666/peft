...
from .layer import PloraLayer, PloraLinear
...
class PloraModel(BaseTuner):
    ...
    def _check_new_adapter_config(self, config: PloraConfig) -> None:
        """
    ...
    def _create_and_replace(
        ...
        **optional_kwargs,
        ...
        pattern_keys = list(chain(plora_config.rank_pattern.keys(), plora_config.alpha_pattern.keys()))
        target_name_key = next(filter(lambda key: re.match(rf".*\.{key}$", current_key), pattern_keys), current_key)
        r = plora_config.rank_pattern.get(target_name_key, plora_config.r)
        ...
        if isinstance(target, PloraLayer):
            target.update_layer(
                adapter_name,
                r,
                plora_config.lora_dropout,
                plora_config.init_weights,
            )
        else:
            new_module = self._create_new_module(plora_config, adapter_name, target, **kwargs)
    ...
    def _create_new_module(fourierft_config, adapter_name, target, **kwargs):
        ...
        new_module = PloraLinear(target, adapter_name, **kwargs)
